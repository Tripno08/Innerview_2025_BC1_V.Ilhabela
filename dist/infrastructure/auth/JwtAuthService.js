...
let JwtAuthService = (() => {
    ...
    var JwtAuthService = class {
        ...
        static {
            ...
            JwtAuthService = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
        generateToken(payload) {
            return (0, jsonwebtoken_1.sign)(payload, auth_1.config.jwt.secret, {
                expiresIn: auth_1.config.jwt.expiresIn,
                subject: payload.sub?.toString(),
            });
        }
        verifyToken(token) {
            try {
                const decoded = (0, jsonwebtoken_1.verify)(token, auth_1.config.jwt.secret);
                return decoded;
            }
            catch (error) {
        ...
        extractTokenFromHeader(authorization) {
            ...
            if (type !== 'Bearer') {
                throw new AppError_1.AppError('Tipo de token inválido', 401);
            }
            return token;
        }
    };
    return JwtAuthService = _classThis;
})();
