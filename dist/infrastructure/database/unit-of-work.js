...
class UnitOfWork {
    ...
    async withTransaction(operation) {
        try {
            ...
            logger_1.logger.error({ error, operation: operation.name }, `Erro na transação: ${error.message}`);
            throw error;
        }
    }
    async withoutTransaction(operation) {
        try {
            ...
            logger_1.logger.error({ error, operation: operation.name }, `Erro na operação: ${error.message}`);
            throw error;
        }
    }
    get prismaClient() {
